...
void Automatico() {
  if (Pir_01_Sens) {
    ...
    if (tempo1 >= 10*60*5 or ((digitalRead(Pir_2) or digitalRead(Pir_3)) and (tempo1 >= 1 * 10*60))) {
  ...
  if (Pir_03_Sens) {
    ...
    if (tempo3 >=  3*10*60 or ((digitalRead(Pir_1) or digitalRead(Pir_2)) and (tempo3 >= 1 * 10*60))) {
...
void reconnect() {
  ...
  if (tiempooff >= 300)
  {
    Cliente = client.connect("NodeMCUMain");
  }
